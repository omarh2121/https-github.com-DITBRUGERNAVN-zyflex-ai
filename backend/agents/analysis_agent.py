...
WEIGHTS = {"weather": 18, "events": 28, "time": 20, "location": 18, "transport_gap": 16}

# Horsens zoner med score for dårlig offentlig transport (0–100)
# Høj score = dårlig dækning = høj taxi-mulighed
TRANSPORT_GAP = {
    "centrum":    10,  # God busdækning
    "station":     5,  # Meget god – tog + bus
    "sygehus":    35,  # Begrænset aftenbus
    "casa_arena": 55,  # Dårlig transport efter events
    "havn":       50,  # Ingen direkte bus aften
    "scandic":    65,  # Udenfor bykernen – dårlig dækning
    # Landsbyer og boligområder med meget dårlig dækning:
    "hatting":    85, "stensballe":  80, "nim":   90, "brædstrup":  75,
    "horsens_nord": 40, "horsens_syd": 45,
}
...
class AnalysisAgent:
    ...
    def run(self, data: dict) -> dict:
        ...
        for zone in zones:
            ...
            tg_score, tg_reasons            = self._transport_gap_score(zone)
            ...
            raw = (
                ...
                tg_score * WEIGHTS["transport_gap"]
            ) / 100
            ...
            all_reasons = w_reasons + e_reasons + t_reasons + l_reasons + tg_reasons + hist_reasons + fb_reasons
    ...
    def _transport_gap_score(self, zone: dict) -> tuple:
        """
        Score baseret på dårlig offentlig transport i zonen.
        Høj score = dårlig busdækning = høj taxi-mulighed.
        Returnerer (score 0-100, reasons).
        """
        zone_id = zone.get("id", "")
        gap = TRANSPORT_GAP.get(zone_id, 30)  # Default: middel dækning

        reasons = []
        if gap >= 75:
            reasons.append(f"Meget dårlig busdækning – taxi er eneste reelle mulighed")
        elif gap >= 55:
            reasons.append(f"Begrænset offentlig transport – høj taxi-efterspørgsel")
        elif gap >= 35:
            reasons.append(f"Sporadisk busservice – taxi konkurrencedygtigt")

        # Ekstra boost aften/nat – busser kører sjældnere
        hour = datetime.now().hour
        if hour >= 21 or hour <= 5:
            gap = min(100, gap + 20)
            if gap >= 50:
                reasons.append(f"Sent om aftenen – busserne kører ikke")
